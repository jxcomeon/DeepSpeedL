...
class DeepSpeedEngine(Module):
    ...
    def _configure_distributed_model(self, model):
        ...
        self.dp_world_size = dist.get_world_size(self.data_parallel_group)
    ...
    @instrument_w_nvtx
    def backward_grad(self, output_obj, output_obj_grad, allreduce_gradients=True, release_loss=False, retain_graph=False, scale_wrt_gas=True):
        ...
        if self.zero_optimization():
            ...
            self.optimizer.backward_grad(output_obj, output_obj_grad, retain_graph=retain_graph)
        elif self.amp_enabled():
            # AMP requires delaying unscale when inside gradient accumulation boundaries
            # https://nvidia.github.io/apex/advanced.html#gradient-accumulation-across-iterations
            delay_unscale = not self.is_gradient_accumulation_boundary()
            with amp.scale_loss(output_obj, self.optimizer, delay_unscale=delay_unscale) as scaled_loss:
                scaled_loss.backward(retain_graph=retain_graph)
        elif self.fp16_enabled():
            if self.eigenvalue_enabled():
                self.optimizer.backward(output_obj, create_graph=True, retain_graph=True)
            else:
                self.optimizer.backward(output_obj, retain_graph=retain_graph)
        elif self.bfloat16_enabled():
            self.optimizer.backward(output_obj)
        else:
            if self.eigenvalue_enabled():
                output_obj.backward(create_graph=True, retain_graph=True)
            else:
                output_obj.backward(retain_graph=retain_graph)
